...
async def bgmi_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Notify the user
        attack_message = (
            f"🚀Hi {user.first_name}🚀, Attack started on {ip}:{port} for {time} seconds\n\n"
        ...
        context.application.job_queue.run_once(
            notify_attack_finished, when=int(time), data={"chat_id": update.effective_chat.id, "ip": ip, "port": port, "user_id": user.id}
        )
